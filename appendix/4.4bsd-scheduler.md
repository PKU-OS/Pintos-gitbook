...
Sets the current thread's nice value to new\_nice and recalculates the thread's priority based on the new value (see section [B.2 Calculating Priority](https://www.cs.jhu.edu/\~huang/cs318/fall21/project/pintos\_8.html#SEC144)). If the running thread no longer has the highest priority, yields.
...
x(1) = f(1),x(2) = a\*f(1) + f(2),...x(5) = a\*\*4\*f(1) + a\*\*3\*f(2) + a\*\*2\*f(3) + a\*f(4) + f(5).
